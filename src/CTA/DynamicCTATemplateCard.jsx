...
export const DynamicCTATemplateCard = ({
  ...
  const renderTitle = () => {
    return (
      <div
        className={`${titleOpacity} ${titleTextClass}`}
  ...
  const renderParagraph = () => {
    ...
    return (
      <p
        className={`${paragraphOpacity} ${paragraphTextClass} ${paragraphExtraClass}`}
  ...
  const RenderConstantStatementModal = () => {
    const onClickFn = async () => {
      //   let postObj = {
      //     cta_type: showConsentDetails?.cta_type,
      //     cta_type_id: "",
      //     target_type: pageName, // page type
      //     target_type_id: id, // page id
      //     position: position,
      //     cta_id: showConsentDetails?.cta_type_id,
      //     };
      ...
      //     const requestData = { ...postObj, utm_source: getUtmSource() };
      //     await axiosInstance.post("/cta/ctareport", requestData);
      //   } catch (error) {
      ...
      >
        {button.map((btn, index) => {
          ...
          return (
            <div
              key={index}
              className={`endorsement-container d-flex align-items-center ${buttonWraper}`}
            >
              <button
                type="button"
                ...
                onClick={() => {
                  if (
                    ...
                    redirectTo(`${[buttonUrl[0]]}/detail`, buttonUrl[1]);
                  } else {
              ...
              >
                <div className="d-flex align-items-center justify-content-center text-nowrap">
                  ...
                  {reactHtmlParser(btn.text || btn.content)}
                  {btn.endEndorment && (
                    <div className="endorsement ms-2">
    ...
    <div className={`d-flex ${flexDirection}`}>
      <div className={`d-flex flex-shrink-0 `}>
        ...
        <img
          src={url}
          alt={alt}
          className={`object-fit-cover ${extraClass}`}
          // ${position !== 'left' ? 'w-100' : ''}
          style={{
      ...
      <div className="p-3">
        ...
        >
          <img
            src={url}
            alt={alt}
            className={`d-flex flex-shrink-0  w-auto mw-25 ${extraClass}`}
            style={{
              height: size || "36px",
            }}
          />
  ...
  return (
    <div
      className={`overflow-hidden mb-4 dynamic_Cta_Main position-relative ${
        cardbgImage ? "position-relative" : ""
      }
    ...
    `}
      ...
      {Array.isArray(formJson) && formJson?.length > 0 && (
        <FormQuestionModal
          show={showFormModal}
          onHide={onHide}
          submitClick={() => toast.success("Form submitted successfully")}
          form={formJson}
        />
      )}
